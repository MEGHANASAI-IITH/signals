...
\author{Manugunta Meghana Sai\\EE23BTECH11212}
\date{}
...
\begin{align}
    F_{s1} &= 5.2-0.3 = 4.9 \text{KHz}\\
    F	_{s2} &= 6.4+0.3 = 6.7  \text{KHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.204 \pi\\
     \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.279 \pi\\
\end{align}
\end{enumerate}
...
$\Omega_{p1} = 0.35417$, $\Omega_{p2} = 0.4453$ and $\Omega_{s1} = 0.332$, $\Omega_{s2} = 0.4688$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.3973$ and $B = \Omega_{p2} - \Omega_{p1} = 0.09113$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.5654\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.4495
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.4495.
\end{align}
...
\input{tables/FDtab1.tex}
we get $N\geq 4$ and $0.3378 \leq \epsilon \leq 0.6197$\\
\newpage
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/MEGHANASAI-IITH/signals/blob/main/filter%20design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/plot1.png}
\caption{The Analog Low-Pass Frequency Response for $0.3378  \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/MEGHANASAI-IITH/signals/blob/main/filter%20design/codes/sk_gen.c
\end{lstlisting}
The poles obtained are formulated in the table below.
\input{tables/FDtab2.tex}
...
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.3125}{\brak{s_L-s_1}\brak{s_L-s_2}\brak{s_L-s_3}\brak{s_L-s_8}}\\
     &= \frac{0.3125}{s_L^4 + 1.1068s_L^3 + 1.6125s_L^2+0.9140s_L + 0.3366}\label{eq:design}
\end{align}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{2.155\times 10^{-5}s^4}{s^8+0.1008s^7+0.644s^6+0.0484s^5+0.1534s^4+0.00763s^3+0.016s^2+0.0004s+0.0006} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
H_{d,BP}(z) &= \frac{G\brak{1 - 4z^{-2} + 6z^{-4} - 4z^{-6} + z^{-8}}}{1.971 - 11.19z^{-1} + 31.34z^{-2} - 54.61z^{-3} + 64.36z^{-4} - 52.29z^{-5} + 28.73z^{-6} - 9.82z^{-7} + 1.65z^{-8}}
\end{align}

where $G=2.155\times 10^{-5}$    
\begin{figure}[H]
\centering
...
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= 2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}\\
            &= \frac{2\cos{\brak{0.242n\pi}}\sin{\brak{0.025n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.242n\pi}}\sin{\brak{0.025n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
